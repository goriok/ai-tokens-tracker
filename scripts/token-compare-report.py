...
<style>
  ...
  .suggestions-heading { color: GrayText; font-size: 11px; text-transform: uppercase; letter-spacing: .02em; margin-bottom: 2px; }
  .suggestion { text-align: left; background: none; border: 1px solid color-mix(in srgb, CanvasText 15%, transparent); border-radius: 4px; padding: 6px 10px; font-size: 13px; }
  .suggestion:hover { background: color-mix(in srgb, CanvasText 6%, transparent); }

  .compare-header { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; margin-bottom: 8px; }
  .compare-header h2 { margin: 0; }
  .compare-header label { display: flex; align-items: center; gap: 4px; font-size: 12px; color: GrayText; }
...
function renderCompareChart() {
  ...
  compareChart = new Chart(document.getElementById("compare-chart"), {
    ...
    options: {
      ...
      scales: {
        x: { stacked: true, title: { display: true, text: "total tokens" } },
        y: { stacked: true },
      },
    },
  });
